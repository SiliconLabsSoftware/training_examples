...
//! **Find Me (Step 6d: Timer) - State Transition Diagram**
//! ```mermaid
//! stateDiagram-v2
...
// BATTERY LIFE:
// CR2032 with 190mAh of power, advertising with no alert
//   defined:      5.7 months (46.4uA) blue led is flashed,     output work runs
//   not defined: 13.4 months (19.5uA) blue led is not flashed, output work paused
//#define APP_ALERT_LEVEL_TARGET_NONE_FLASH_LED
...
#define APP_STATE_LIST(X)          \
	...
	X(APP_STATE_CLR_CONNECTING)    \
	X(APP_STATE_CLR_WRITING_ALC)   \
	X(APP_STATE_CLR_DISCONNECTING)
...
#ifdef APP_ALERT_LEVEL_TARGET_NONE_FLASH_LED
	{false, false, true, 0, 50, 1450}, // APP_ALERT_LEVEL_TARGET_NONE
#else
	{false, false, false, 0, 50, 1450}, // APP_ALERT_LEVEL_TARGET_NONE
#endif
...
// Target handle stack (these devices have had an attempted write and should be cleared)
typedef struct
{
	bt_addr_le_t addr;
	uint16_t handle;
...
static target_handle_t target_handle_stack[TARGET_STACK_MAX];
static uint8_t target_handle_stack_count = 0;
...
{
#ifndef APP_ALERT_LEVEL_TARGET_NONE_FLASH_LED
	bool output_work_resume = false;

	// In alert level target none state ?
	if (app_alert_level == APP_ALERT_LEVEL_TARGET_NONE)
	{
		// Moving to idle state ?
		if (state == APP_STATE_IDLE &&
			app_state != APP_STATE_IDLE)
		{
			// Need to resume output work
			output_work_resume = true;
		}
	}
#endif
...
#ifndef APP_ALERT_LEVEL_TARGET_NONE_FLASH_LED
	// Need to resume output work ?
	if (output_work_resume)
	{
		// Fiddle to off state so led gets turned on
		output_is_on = false;
		// Resume output work immediately
...
static inline void app_alert_level_set(app_alert_level_t alert_level)
{
	...
	{
		// Moving away from target none alert ?
		if (app_alert_level == APP_ALERT_LEVEL_TARGET_NONE &&
			alert_level != APP_ALERT_LEVEL_TARGET_NONE)
		{
			// Need to resume output work
			output_work_resume = true;
		}
...
static void output_work_handler(struct k_work *work)
{
	...
	bool output_work_schedule = true;

#ifndef APP_ALERT_LEVEL_TARGET_NONE_FLASH_LED
	// Not in idle state ?
	if (app_state != APP_STATE_IDLE)
	{
		// In target none alert level ?
		if (app_alert_level == APP_ALERT_LEVEL_TARGET_NONE)
		{
			// Allow output work to pause
			output_work_schedule = false;
		}
	}
#endif
	...
	if (output_is_on)
	{
		...
		if (output_work_schedule) k_work_schedule(&output_work, K_MSEC(output_config[output_config_index].on_ms));
	}
	// Turn outputs off ?
	else
	{
		...
		if (output_work_schedule) k_work_schedule(&output_work, K_MSEC(output_config[output_config_index].off_ms));
	}
}
...
static void target_handle_stack_clear(void)
{
	memset(target_handle_stack, 0, sizeof(target_handle_stack));
	target_handle_stack_count = 0;
}

static bool target_handle_stack_check(const bt_addr_le_t *addr)
{
	// List is full, fail the check
	if (target_handle_stack_count >= TARGET_STACK_MAX)
		return false;
	// Address is in list, fail the check
	for (uint8_t i = 0; i < target_handle_stack_count; i++)
	{
		if (bt_addr_le_cmp(addr, &target_handle_stack[i].addr) == 0)
		{
			return false;
		}
	}
...
static bool target_handle_stack_push(const bt_addr_le_t *addr, const uint16_t handle)
{
	// Can add to list ?
	if (target_handle_stack_check(addr))
	{
		bt_addr_le_copy(&target_handle_stack[target_handle_stack_count].addr, addr);
		target_handle_stack[target_handle_stack_count].handle = handle;
		target_handle_stack[target_handle_stack_count].clear = 3;
		char addr_str[BT_ADDR_LE_STR_LEN];
		bt_addr_le_to_str(&target_handle_stack[target_handle_stack_count].addr, addr_str, sizeof(addr_str));
		target_handle_stack_count++;
		return true;
	}
...
static bool target_handle_stack_pop(bt_addr_le_t *addr, uint16_t *handle)
{
	// Got something in list ?
	if (target_handle_stack_count > 0)
	{
		bt_addr_le_copy(addr, &target_handle_stack[target_handle_stack_count - 1].addr);
		*handle = target_handle_stack[target_handle_stack_count - 1].handle;
		if (target_handle_stack[target_handle_stack_count - 1].clear > 0)
		{
			target_handle_stack[target_handle_stack_count - 1].clear--;
		}
		if (target_handle_stack[target_handle_stack_count - 1].clear == 0)
		{
			target_handle_stack_count--;
...
// Go to CLR_CONNECTING or ADVERTISING handler function
static int app_state_to_clr_connecting_or_advertising(void)
{
	int err;

	// Do we need to clear an alert from a device ?
	if (target_handle_stack_pop(&target_addr, &handle_characteristic_value))
	{
		// Go to locator alert level none
		app_alert_level_set(APP_ALERT_LEVEL_LOCATOR_NONE);
		...
		if (!err)
		{
			...
			app_state_set(APP_STATE_CLR_CONNECTING);
		}
	}
	// No devices to clear ?
	else
...
static int app_event_scanning(app_event_t app_evt)
{
	...
	if (app_evt < APP_EVENT_MAX)
	{
		...
		case APP_EVENT_PRESSED_SW1:
			...
			(void)app_scan_stop();
			// Go to CLR_CONNECTING or ADVERTISING handler function
			err = app_state_to_clr_connecting_or_advertising();
			break;
...
static int app_event_discovering_alc(app_event_t app_evt)
{
	...
	if (app_evt < APP_EVENT_MAX)
	{
		...
		case APP_EVENT_DISCOVER_CHARACTERISTIC_OK:
			...
			if (!err)
			{
				// Add to handle list so we can clear alerts later
				target_handle_stack_push(bt_conn_get_dst(bt_current_conn), handle_characteristic_value);
				// Go to writing state
...
// App event for clear connecting state
static int app_event_clr_connecting(app_event_t app_evt)
{
	int err = 0;
	...
	if (app_evt < APP_EVENT_MAX)
	{
		printk("app_event_clr_connecting(%s)\n", app_event_strings[app_evt]);
		// Which event ?
		switch (app_evt)
		{
		...
		case APP_EVENT_CONNECTED_CENTRAL:
			// Attempt to write alert level
			err = app_write_alert_level(0);
			// Success ?
			if (!err)
			{
				// Go to writing state
				//! CLR_CONNECTING --> CLR_WRITING_ALC: connected_central
				app_state_set(APP_STATE_CLR_WRITING_ALC);
				// Schedule timeout
				k_work_schedule(&bt_timeout_work, K_MSEC(TIMEOUT_MS));
			}
			// Fail, clear error and disconnect
	...
	{
		printk("app_event_clr_connecting(%u) {UNKNOWN}\n", app_evt);
	}

	return err;
}

// App event for clear writing ALC state
static int app_event_clr_writing_alc(app_event_t app_evt)
{
	int err = 0;
	...
	if (app_evt < APP_EVENT_MAX)
	{
		printk("app_event_clr_writing_alc(%s)\n", app_event_strings[app_evt]);
		// Which event ?
		switch (app_evt)
		{
		...
		case APP_EVENT_TIMEOUT:
			...
			if (!err)
			{
				// Go to clear disconnecting central state
				//! CLR_WRITING_ALC --> CLR_DISCONNECTING: timeout
				app_state_set(APP_STATE_CLR_DISCONNECTING);
			}
			break;
		...
		case APP_EVENT_DISCONNECTED_CENTRAL:
			// Cancel timeout
			k_work_cancel_delayable(&bt_timeout_work);
			// Go to CLR_CONNECTING or ADVERTISING
			err = app_state_to_clr_connecting_or_advertising();
			break;
	...
	{
		printk("app_event_clr_writing_alc(%u) {UNKNOWN}\n", app_evt);
	}

	return err;
}

// App event for clear disconnecting state
static int app_event_clr_disconnecting(app_event_t app_evt)
{
	int err = 0;
	...
	if (app_evt < APP_EVENT_MAX)
	{
		printk("app_event_clr_disconnecting(%s)\n", app_event_strings[app_evt]);
		// Which event ?
		switch (app_evt)
		{
		case APP_EVENT_DISCONNECTED_CENTRAL:
	...
	{
		printk("app_event_clr_disconnecting(%u) {UNKNOWN}\n", app_evt);
	}
		...
		if (!app_event_consumed)
		{
			...
			if (app_state < APP_STATE_MAX)
			{
				...
				case APP_STATE_SET_DISCONNECTING:
					...
					break;
				case APP_STATE_CLR_CONNECTING:
					err = app_event_clr_connecting(app_evt);
					break;
				case APP_STATE_CLR_WRITING_ALC:
					err = app_event_clr_writing_alc(app_evt);
					break;
				case APP_STATE_CLR_DISCONNECTING:
					err = app_event_clr_disconnecting(app_evt);
					break;
				default:
